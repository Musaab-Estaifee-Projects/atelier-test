...
const SelectStep = ({
  ...
  const handleSuggestion = (unitId: string) => {
    const unit = CATALOG_UNITS.find((item) => item.id === unitId);
    if (!unit) return;
    form.setValue("query", unit.id);
    form.setValue("typeId", unit.typeId);
    form.setValue("layoutId", unit.layoutId);
    submitChoice({ unitId: unit.unitId, levelName: unit.levelName });
  };
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <DashedDropdown
              ...
              onChange={(id) => {
                const layout = LAYOUTS.find((item) => item.id === id);
                form.setValue("layoutId", id);
                if (layout) form.setValue("typeId", layout.typeId);
                setLocalError(null);
                setOpenMenu(null);
              }}
            />
